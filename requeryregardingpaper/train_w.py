...
model.summary();
labels1 = np.genfromtxt('./Fold_2/lists/train_label_w.lst', dtype='int32')
...
callback2=tensorflow.keras.callbacks.ModelCheckpoint('./Fold_2/Models_w/checkpoint-phn_emb-{epoch:02d}-{val_accuracy:.4f}.hdf5', monitor='val_accuracy', verbose=1, save_best_only=True, save_weights_only=True, mode='auto', period=1)
print('..fit model')	
history=model.fit_generator(DataGenerator('./Fold_2/lists/train_data_w.lst','./Fold_2/lists/train_label_w.lst'),epochs=10,validation_data=DataGenerator('./Fold_2/lists/val_data_w.lst','./Fold_2/lists/val_label_w.lst'),callbacks=[callback1,callback2], workers=20, use_multiprocessing=False)
